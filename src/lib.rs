...
impl StrIsEnumMember for WordTransformation {
    ...
    fn into_iter() -> impl Iterator<Item = (&'static str, Self)> {
        [
            ...
            (Self::CapitalizeNotFirst.to_static_str(), Self::CapitalizeNotFirst),
            (Self::AlternatingLowerUpper.to_static_str(), Self::AlternatingLowerUpper),
            (Self::AlternatingUpperLower.to_static_str(), Self::AlternatingUpperLower),
            (Self::RandomUpperLower.to_static_str(), Self::RandomUpperLower),
        ].into_iter()
    }
    fn to_member(name: &str) -> Result<Self, ()> {
        Self::into_iter().find(|(s, _)| *s == name).map(|inner| inner.1).ok_or(())
    }
}
...
where T: Rng + Default {
    ...
    fn filter_wordlist(&self) -> Vec<u32> {
        ...
        let filtered_indices: Vec<u32> = self.wordlist.iter().enumerate().filter(|(_, word)| word.chars().count() >= min_len && word.chars().count() <= max_len).map(|(i, _)| i as u32).collect();
        filtered_indices
    }
    fn choose_words(&mut self, indices: &[u32]) -> Result<Vec<String>, String> {
        ...
        let result = selected.into_iter().map(|n| self.wordlist[*n as usize].clone()).collect();
        Ok(result)
    }
    fn transform_words(&mut self, words: Vec<String>) -> Result<Vec<String>, String> {
        ...
        Ok(
            match self.config.word_transformation {
                ...
                WordTransformation::AlternatingLowerUpper => word_transformer::alternating_lower_upper(words),
                WordTransformation::AlternatingUpperLower => word_transformer::alternating_upper_lower(words),
                WordTransformation::RandomUpperLower => word_transformer::random_upper_lower(&mut self.rng, words),
            }
        )
    }
    ...
    fn choose_separator(&mut self) -> Option<char> {
        Some(*self.config.separator_character.choose(&mut self.rng).unwrap())
    }
    ...
    fn create_password(&mut self) -> Result<String, String> {
        ...
        let final_password = format!("{}{}{}", front_padding.unwrap_or("".to_owned()), unpadded_password, rear_padding.unwrap_or("".to_owned()));
        Ok(final_password)
    }
...
mod password_maker_tests {
    ...
    fn test_filter_wordlist() {
        ...
        let expected = [5, 6];
        let matches = result.iter().zip(expected.iter()).filter(|&(a, b)| a == b).count();
